#!/usr/bin/env python
# 使用本地/远端 Reranker 对候选论文做重排序（简化版）。
# 显式配置 DPR_SKIP_RERANK=true 时，会使用召回分数生成兜底排序。
...
from llm import BltClient, first_env
...
def _resolve_remote_api_key(provider: str) -> str:
  ...
  return first_env("RERANK_API_KEY", "RERANKER_API_KEY", "Reranker_LLM_API_KEY")
...
def apply_rerank_fallback(input_path: str, output_path: str, reason: str) -> None:
  ...
  log(f"[INFO] 跳过 rerank，使用召回分数兜底排序：{reason}")
...
def main() -> None:
  ...
  parser.add_argument(
    "--rerank-model",
    type=str,
    default=first_env("RERANK_MODEL", "RERANKER_MODEL", "Reranker_LLM_MODEL"),
